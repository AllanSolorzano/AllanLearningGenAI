...
- Understand embeddings, cosine similarity, BM25, reciprocal rank fusion, and reranking
- Extract and clean document text while preserving provenance
- Design metadata schemas for filtering, citations, freshness, and debugging
- Compare chunking strategies: fixed-size, heading-aware, overlap, and merge-small
- Build vector, keyword, hybrid, and metadata-filtered retrieval
- Rerank candidates before sending context to an LLM
- Assemble grounded prompts with citations and safe "not found" behavior
- Evaluate RAG with retrieval and answer metrics

## Pipeline

```
Documents -> pages -> cleaned pages -> heading-aware token chunks
          -> merged chunks -> embeddings -> vector store
          -> retrieve top 20 -> rerank top 5 -> answer with citations
```

Shared helpers live in `shared/rag_utils.py` (used by solutions and `allan_project`).
...
| lab04_retrieval_reranking | Hybrid retrieval | keyword, vector, RRF, reranking |
...
| Demo | What it shows |
|------|---------------|
| `demo_chunking_strategies.py` | Chunk size, overlap, headings, and metadata affect retrieval |
| `demo_retrieval_and_reranking.py` | Vector, keyword, hybrid retrieval, and reranking |
| `demo_chroma_rag_pipeline.py` | Full local pipeline with Chroma and sentence-transformers |
...
## Allan project integration

The `allan_project` agent uses FTS over durable memory today. Session 07 adds
hybrid retrieval (FTS + vectors + RRF) in `allan_ollama_mcp/rag/`:
